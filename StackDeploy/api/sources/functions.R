...
## Checks if connection is closed for a db and tells if reconnection is needed.
.NeedsReconnection <- function(conn) {
  tryCatch({
    dbSendQuery(conn, "SELECT 1;")
  },
  error = function(err) {
    assign("check", FALSE, envir = .bit.env)
    error1 <-
      "Error in .jcall(conn@jc, \"Ljava/sql/Statement;\", \"createStatement\"): java.sql.SQLNonTransientConnectionException: No operations allowed after connection closed.\n"
    assign("check", as.character(err) == error1, envir = .bit.env)
    return(.bit.env$check)
  })
  needs <- ifelse(.bit.env$check == TRUE, TRUE, FALSE)
  .bit.env$check <- FALSE
  return(needs)
}

# Checks if db connection exists but closed and reopens it.
.ReconnectOnClose <- function(db) {
  if (.NeedsReconnection(db) == TRUE) {
    assign("conn.name", deparse(substitute(db)), envir = .obj.env)
    eval(parse(
      text = paste("rm(", .obj.env$conn.name, ", envir = .GlobalEnv)", sep = "")
    ))
    eval(parse(
      text = paste(
        "assign('",
        .obj.env$conn.name,
        "', .ConnectToDatabase() ,envir = .GlobalEnv)",
        sep = ""
      )
    ))
  } else {
    message("Doesn't need reconnection or connection doesn't already exist")
  }
}
